...
{
    public class Apoderado : Usuario
    {
        public override bool Delete(string ERut)
        {
            using (OracleConnection ora = new OracleConnection(connection))
            {
                ...
                {
                    ora.Open();
                    OracleCommand command = new OracleCommand("DELETE FROM APODERADO WHERE RUT =:RUT", ora);
        ...
        public override bool Insert(string NRut, string NEmail, string NPass, string NNombre, string NApellido)
        {
            using (OracleConnection ora = new OracleConnection(connection))
            {
                ...
                {
                    ora.Open();
                    OracleCommand command = new OracleCommand("INSERT INTO APODERADO VALUES (:RUT, :EMAIL, :PASS, :NOMBRE, :APELLIDO)",ora);
        ...
        public override Usuario Select()
        {
            ...
            using (OracleConnection ora = new OracleConnection(connection))
            {
                ...
                {
                    ora.Open();
                    OracleCommand command = new OracleCommand("SELECT * FROM APODERADO", ora);
        ...
        public override bool SelectInicioSesion()
        {
            ...
            using (OracleConnection ora = new OracleConnection(connection))
            {
                ...
                {
                    ora.Open();
                    OracleCommand command = new OracleCommand("SELECT * FROM APODERADO WHERE RUTAPODERADO=:RUT", ora);
        ...
        public override Usuario SelectWhere(string Rut)
        {
            ...
            using (OracleConnection ora = new OracleConnection(connection))
            {
                ...
                {
                    ora.Open();
                    OracleCommand command = new OracleCommand("SELECT * FROM USUARIO WHERE RUT=:RUT", ora);
        ...
        public static Apoderado AutoCompletar(string Rut)
        {
            Apoderado apo = null;
            using (OracleConnection ora = new OracleConnection(connection))
            {
                try
                {
                    ...
                    if (lector.Read())
                    {
                        apo = new Apoderado { Email = lector.GetString(1), Nombre = lector.GetString(3), Apellido = lector.GetString(4) };
                    }
                }
                catch (Exception ex)
                {
